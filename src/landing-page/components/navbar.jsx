...
const navigation = [
    { title: "Courses", path: "#", isDrapdown: true, navs: dropdownNavs },
...
const Navbar = () => {
    ...
    return (
        <>
            <nav className={`z-20 fixed top-0 border-b border-black/30 bg-white w-full md:text-sm ${state ? "shadow-lg rounded-b-xl md:shadow-none" : ""}`}>
                <div className="items-center gap-x-14 px-4 max-w-screen-xl mx-auto md:flex md:px-8">
                    ...
                    <div className={`nav-menu flex-1 pb-3 mt-8 z-40 md:block md:pb-0 md:mt-0 ${state ? 'block' : 'hidden'}`}>
                            ...
                            {navigation.map((item, idx) => (
                                <li key={idx}>
                                    {item.isDrapdown ? (
                                        ...
                                        <Link to={item.path} smooth={true} duration={500} className="block cursor-pointer text-gray-700 hover:text-indigo-600">
                                            {item.title}
                                        </Link>
                                    )}
                                    {item.isDrapdown && drapdownState.idx === idx && drapdownState.isActive ? (
                                        <div className="mt-6 bg-white inset-x-0 top-[4rem] w-full md:absolute md:border-y md:shadow-md md:mt-0">
